...
Shopware.Component.override('sw-media-index', {
    template,
    ...
    methods: {
        ...
        //ToDo fix returns no Request Payload
        getConfig() {
    	...
    	onTranslate: function() {
    		...
    		this.httpClient.post(
                ...
                {entity: 'media', entityId: this.routeFolderId, languageId: this.languageStore.getCurrentId()},
                {headers: this.syncService.getBasicHeaders() }
            ).then((response) => {
                ...
                if(response.data.status && response.data.status == 'error') {
                    ...
                    for(let key in response.data) {
                        this.uploads[key] = response.data[key];
                    }
