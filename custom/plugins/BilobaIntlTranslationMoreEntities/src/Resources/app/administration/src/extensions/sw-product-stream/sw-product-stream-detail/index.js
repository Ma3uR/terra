...
Shopware.Component.override('sw-product-stream-detail', {
    template,
    ...
    methods: {
    	...
    	onTranslate: function() {
    		...
    		this.httpClient.post(
                ...
                {entity: 'product_stream', entityId: this.productStream.id, languageId: this.languageStore.getCurrentId()},
                {headers: this.syncService.getBasicHeaders() }
            ).then((response) => {
                ...
                if(response.data.status && response.data.status == 'error') {
                    ...
                    for(let key in response.data) {
                        this.productStream[key] = response.data[key];
                    }
