...
use Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\TaskEvents\AliveAnnouncedTaskEvent;
use Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\TaskEvents\ProgressedTaskEvent;
...
abstract class CompositeTask extends Task
{
    ...
    {
        /** @var static $entity */
        $entity = new static(array(), $array['initialProgress']);

        $entity->taskProgressMap = $array['taskProgress'];
        $entity->subTasksProgressShare = $array['subTasksProgressShare'];

        foreach ($array['tasks'] as $taskKey => $task) {
            $entity->tasks[$taskKey] = Serializer::unserialize($task);
        }

        $entity->attachSubTasksEvents();
    ...
    public function toArray()
    {
        ...
        foreach ($this->tasks as $taskKey => $task) {
            $tasks[$taskKey] = Serializer::serialize($task);
        }
     ...
     *
     * @return \Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\Task|null
     ...
     *
     * @param \Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\Task $task Task object.
     */
