...
class DocumentFileSizeMigrator
{
    ...
    public function __construct(Connection $connection, FilesystemInterface $privateFileSystem)
    {
    ...
    public function migrateFileSize(): void
    {
        ...
        foreach ($filesWithNoFileSize as $fileWithNoFileSize) {
            $size = 0;
            if ($this->privateFileSystem->has($fileWithNoFileSize['pathInPrivateFileSystem'])) {
                $size = $this->privateFileSystem->getSize($fileWithNoFileSize['pathInPrivateFileSystem']);
            }
