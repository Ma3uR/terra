...
class Migration1606220870CreateStockValuationView extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
        ...
        $extractPurchasePriceString = 'COALESCE(
            JSON_UNQUOTE(JSON_EXTRACT(product.purchase_prices, "$.' . $defaultCurrencyKey . '.%1$s")),
            JSON_UNQUOTE(JSON_EXTRACT(
                product.purchase_prices,
                CONCAT("$.", JSON_UNQUOTE(JSON_EXTRACT(JSON_KEYS(product.purchase_prices), "$[0]")), ".%1$s")
            ))
        )';
        $extractPurchasePriceCurrencyString = 'UNHEX(IF(
            JSON_EXTRACT(product.purchase_prices, "$.' . $defaultCurrencyKey . '") IS NOT NULL,
            "' . Defaults::CURRENCY . '",
            SUBSTR(JSON_UNQUOTE(JSON_EXTRACT(JSON_KEYS(product.purchase_prices), "$[0]")), 2)
        ))';
