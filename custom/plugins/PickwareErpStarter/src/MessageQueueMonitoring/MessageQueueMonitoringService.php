...
class MessageQueueMonitoringService
{
     ...
     * @var bool
     */
    private $isAdminWorkerActive;

    public function __construct(Connection $connection, bool $isAdminWorkerActive)
    {
    ...
    public function logCLIWorkerRun(): void
    {
        $this->connection->executeStatement(
            'INSERT INTO message_queue_monitoring (
                id,
                last_cli_worker_run
            ) VALUES (
                UNHEX(:loggingId),
    ...
    public function getStatus(): string
    {
        $lastLogData = $this->connection->fetchAssociative(
            'SELECT last_cli_worker_run, last_administration_login FROM message_queue_monitoring WHERE id=UNHEX(:loggingId);',
            [ 'loggingId' => self::LOGGING_ID ]
        );
