...
class DocumentEntityWriteValidator implements EventSubscriberInterface
{
    ...
    {
        $documentCommands = array_values(array_filter($event->getCommands(), function (WriteCommand $command) {
            return $command->getDefinition()->getClass() === DocumentDefinition::class && !($command instanceof DeleteCommand);
        }));
